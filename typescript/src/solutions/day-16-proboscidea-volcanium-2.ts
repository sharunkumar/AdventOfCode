...
type valve = string
type rate = number
interface distance { [key: valve]: { [key: valve]: rate } }
interface valves { [key: valve]: rate }
interface tunnels { [key: valve]: valve[] }
interface indices { [key: valve]: number }
...
export default class ProboscideaVolcanium extends Solution {
    solve(input: string) {
        ...
        for (let i = 0; i < (b + 1) / 2; i++) {
            global_max = Math.max(global_max, dfs(26, "AA", i) + dfs(26, "AA", b ^ i))
        }
