import { ascending, pipelog, Solution, spreadMap, sum } from "../utils";
...
export default class NoSpaceLeftOnDevice extends Solution {
    ...
    solve(input: string) {
        let commands = this.get_lines(input)
            .map(line => line.split(" "))
        // .map(pipelog)
        ...
        const total_disk_size = 70000000, required_free_space = 30000000
        ...
        return this.rootDirectory.getCandidates(required_free_space)
            .map(dir => dir.getSize())
