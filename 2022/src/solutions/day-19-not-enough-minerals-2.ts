...
export default class NotEnoughMinerals extends Solution {
  solution(
    ...
    let max_yeild =
      remaining * state.pack.geode_robot +
      Math.floor((remaining * (remaining - 1)) / 2)
    ...
    if (can_build_geode_robot(state, bp)) {
      this.solution(
        build_geode_robot(step(state), bp),
        bp,
        limit,
        rez,
        true,
      ...
      if (can_build_obsidian_robot(state, bp)) {
        ...
        if (
          can_obsidian &&
          state.pack.obsidian_robot < bp.max_obsidian_cost()
        ) {
          this.solution(
            build_obsidian_robot(step(state), bp),
            bp,
            limit,
            rez,
            true,
      ...
      if (can_build_clay_robot(state, bp)) {
        ...
        if (can_clay && state.pack.clay_robot < bp.max_clay_cost()) {
          this.solution(
            build_clay_robot(step(state), bp),
            bp,
            limit,
            rez,
            true,
  ...
  solve(input: string) {
    let blueprints = this.get_lines(input)
      .map(
        (line) =>
          regexMatch(line, /\d+/g).map(Number) as [
            number,
            number,
            number,
            number,
